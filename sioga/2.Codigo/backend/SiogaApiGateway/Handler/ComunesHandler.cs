...
{
    public class ComunesHandler : DelegatingHandler
    {
        private readonly ILogger<ComunesHandler> _logger;
        private readonly AppSettings _appSettings;
        private readonly IAutorizationService _autorizationService;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ComunesHandler(
            IHttpContextAccessor httpContextAccessor,
            AppSettings appSettings,
            ILogger<ComunesHandler> logger,
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (request.Method == HttpMethod.Post || request.Method == HttpMethod.Put)
                {
                    ...
                    var rolesSubvencion = _appSettings.Rol.Subvencion;

                    var rolesCodeApp = new List<string>();
                    rolesCodeApp.AddRange(rolesRecaudacion);
                    rolesCodeApp.AddRange(rolesSubvencion);
