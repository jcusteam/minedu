...
{
    public class KardexAlmacenHandler
    {
        public class StatusKardexResponse : StatusResponse<FileContent>
        {

        }
        public class Command : IRequest<StatusKardexResponse>
        {
            public KardexAlmacenDto KardexAlmacenDto { get; set; }
        }

        public class Handler : IRequestHandler<Command, StatusKardexResponse>
        {
            ...
            public async Task<StatusKardexResponse> Handle(Command request, CancellationToken cancellationToken)
            {
                var response = new StatusKardexResponse();
                ...
                try
                {
                    ...
                    var kardexAlmacen = request.KardexAlmacenDto;

                    var memory = new MemoryStream();
                    var fileName = _appSettings.Plantilla.KardexAlmacen;
                    ...
                    template.AddVariable(kardexAlmacen);
                    template.Generate();
