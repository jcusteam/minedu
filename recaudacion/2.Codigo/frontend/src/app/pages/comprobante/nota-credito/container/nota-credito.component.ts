...
import { InfoNotaCreditoComponent } from '../dialogs/info-nota-credito/info-nota-credito.component';
import { NewNotaCreditoComponent } from '../dialogs/new-nota-credito/new-nota-credito.component';
import { ComprobantePago, ComprobantePagoEstado, ComprobantePagoFilter } from 'src/app/core/models/comprobantepago';
...
import { AccionEnum, EstadoComprobantePagoEnum, FileServerEnum, MenuEnum, TipoAdquisicionEnum, TipoCaptacionEnum, TipoComprobanteEnum, TipoDocEnum, TipoDocIdentidadEnum, TipoOperacionEnum } from 'src/app/core/enums/recaudacion.enum';
...
export class NotaCreditoComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.filter.tipoDocumentoId = this.tipoDocEnum.NOTA_CREDITO;
  ...
  onlaodAccion() {
    this.authService.getAcciones(this.menuEnum.NOTA_CREDITO)
      .subscribe(
        (response) => {
          if (response.success) {
  ...
  openDialogNew(): void {
    ...
    comprobantePago.tipoDocumentoId = this.tipoDocEnum.NOTA_CREDITO;
    comprobantePago.tipoComprobanteId = this.tipoComprobanteEnum.NOTA_CREDITO;
    ...
    const dialogRef = this.dialog.open(NewNotaCreditoComponent, {
      width: "1200px",
      disableClose: true,
  ...
  openDialogInfo(data): void {
    ...
    const dialogRef = this.dialog.open(InfoNotaCreditoComponent, {
      width: "1200px",
      data: data,
    });
    dialogRef.afterClosed().subscribe(() => {
