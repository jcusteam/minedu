...
import { ListSaldoIngresoNotaCreditoComponent } from "../list-saldo-ingreso/list-saldo-ingreso.component";
import { NewNotaCreditoClienteComponent } from "../new-nota-credito-cliente/new-nota-credito-cliente.component";
...
@Component({
  selector: "app-new-nota-credito",
  templateUrl: "./new-nota-credito.component.html",
  styleUrls: ["./new-nota-credito.component.scss"],
  providers: [
    { provide: DatePipe },
...
export class NewNotaCreditoComponent implements OnInit {
  // Enums
  ...
  // Constructor
  constructor(
    public dialogRef: MatDialogRef<NewNotaCreditoComponent>,
  ...
  selectedChangeFuenteOrigen(id) {
    if (id == this.fuenteOrigenEnum.INTERNO) {
      ...
      this.form.patchValue({
        fuenteId: 0,
        fuenteValidar: this.fuenteValidaEnum.PENDIENTE,
        documentoFuenteValida: false
      });
    }
    else {
  ...
  searchDocFuente() {
    ...
    comprobantePagoFuente.unidadEjecutoraId = unidadEjecutoraId;
  ...
  openDialogCatalogoBien() {
    ...
    const dialogRef = this.dialog.open(ListSaldoIngresoNotaCreditoComponent, {
      width: "1000px",
      disableClose: true,
      data: catalogoBien,
    });
  ...
  onSubmit(form: ComprobantePago) {
    ...
    if (this.dataSource.data.length == 0) {
      this.messageService.msgWarning("Ingrese el detalle de la Nota de Crédito", () => { });
      return;
    }
  ...
  openDialogNewCliente() {
    ...
    const dialogRef = this.dialog.open(NewNotaCreditoClienteComponent, {
      width: "800px",
      disableClose: true,
      data: cliente,
    });
