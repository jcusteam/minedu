...
import { InfoNotaDebitoComponent } from '../dialogs/info-nota-debito/info-nota-debito.component';
import { NewNotaDebitoComponent } from '../dialogs/new-nota-debito/new-nota-debito.component';
...
export class NotaDebitoComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.filter.tipoDocumentoId = this.tipoDocEnum.NOTA_DEBITO;
  ...
  onlaodAccion() {
    this.authService.getAcciones(this.menuEnum.NOTA_DEBITO)
      .subscribe(
        (response) => {
          if (response.success) {
  ...
  openDialogNew(): void {
    ...
    comprobantePago.tipoDocumentoId = this.tipoDocEnum.NOTA_DEBITO;
    comprobantePago.tipoComprobanteId = this.tipoComprobanteEnum.NOTA_DEBITO;
    ...
    const dialogRef = this.dialog.open(NewNotaDebitoComponent, {
      width: "1200px",
      disableClose: true,
  ...
  openDialogInfo(data): void {
    ...
    const dialogRef = this.dialog.open(InfoNotaDebitoComponent, {
      width: "1200px",
      data: data,
    });
    dialogRef.afterClosed().subscribe(() => {
