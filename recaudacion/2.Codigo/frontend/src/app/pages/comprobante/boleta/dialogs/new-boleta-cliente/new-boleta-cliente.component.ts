...
@Component({
  selector: "app-new-boleta-cliente",
  templateUrl: "./new-boleta-cliente.component.html",
  styleUrls: ["./new-boleta-cliente.component.scss"],
})
export class NewBoletaClienteComponent implements OnInit {
  //Enums
  ...
  constructor(
    public dialogRef: MatDialogRef<NewBoletaClienteComponent>,
  ...
  ngOnInit() {
    this.tipoDocumentoIdentidadSercice.getTipoDocumentoIdentidades().subscribe(
      (response) => {
        if (response.success) {
          this.tipoDocumentos = response.data.filter(x => x.tipoDocumentoIdentidadId == this.tipoDocIdentidadEnum.DNI ||
            x.tipoDocumentoIdentidadId == this.tipoDocIdentidadEnum.CE);
        }
      });
  }
