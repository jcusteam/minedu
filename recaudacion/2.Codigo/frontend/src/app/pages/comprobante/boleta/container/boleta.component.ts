...
import { InfoBoletaComponent } from "../dialogs/info-boleta/info-boleta.component";
import { NewBoletaComponent } from "../dialogs/new-boleta/new-boleta.component";
...
export class BoletaComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.filter.tipoDocumentoId = this.tipoDocEnum.BOLETA_VENTA;
  ...
  onlaodAccion() {
    this.authService.getAcciones(this.menuEnum.BOLETA)
      .subscribe(
        (response) => {
          if (response.success) {
  ...
  openDialogNew(): void {
    ...
    comprobantePago.tipoDocumentoId = this.tipoDocEnum.BOLETA_VENTA;
    comprobantePago.tipoComprobanteId = this.tipoComprobanteEnum.BOLETA_VENTA;
    ...
    const dialogRef = this.dialog.open(NewBoletaComponent, {
      width: "1200px",
      disableClose: true,
  ...
  openDialogInfo(data): void {
    ...
    const dialogRef = this.dialog.open(InfoBoletaComponent, {
      width: "1200px",
      data: data,
    });
    dialogRef.afterClosed().subscribe(() => {
