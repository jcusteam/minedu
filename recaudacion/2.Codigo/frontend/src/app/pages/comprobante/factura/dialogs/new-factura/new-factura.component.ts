...
import { ComprobantePago, ComprobantePagoDetalle, ComprobantePagoFuente } from "src/app/core/models/comprobantepago";
...
import { ListSaldoIngresoFacturaComponent } from "../list-saldo-ingreso/list-saldo-ingreso.component";
import { CuentaCorrienteService } from "src/app/core/services/cuenta-corriente.service";
import { NewFacturaClienteComponent } from "../new-factura-cliente/new-factura-cliente.component";
...
import { FuenteOrigenEnum, FuenteValidaEnum, IGVEnum, TipoAdquisicionEnum, TipoCaptacionEnum, TipoComprobanteEnum, TipoDocEnum, TipoDocIdentidadEnum, TipoIGVEnum, TipoMonedaEnum, TipoOperacionEnum, TipoPrecioVentaEnum, ValidaDepositoEnum } from "src/app/core/enums/recaudacion.enum";
...
export class NewFacturaComponent implements OnInit {
  ...
  // Constructor
  constructor(
    public dialogRef: MatDialogRef<NewFacturaComponent>,
  ...
  ngOnInit() {
    ...
    this.valiationTipoDocIdentidad(this.tipoDocIdentidadEnum.RUC)
  }
  ...
  searchDocFuente() {
    ...
    this.comprobantePagoService.getComprobantePagoByFuente(comprobantePagoFuente).subscribe(
      response => {
        ...
        if (response.success) {
          this.form.patchValue({
            fueneteId: response.data.comprobantePagoId,
  ...
  openDialogCatalogoBien() {
    ...
    const dialogRef = this.dialog.open(ListSaldoIngresoFacturaComponent, {
      width: "1000px",
      disableClose: true,
      data: catalogoBien,
    });
    ...
    dialogRef.afterClosed().subscribe((response: IngresoPecosaDetalle[]) => {
      if (response.length > 0) {
        response.filter(x => x.cantidadSalida > 0 && x.serieAlSalida > 0 && x.serieAlSalida > 0).forEach((item) => {
          ...
          detalle.totalItemSinIGV = item.precioUnitario;
          detalle.codigoTipoPrecio = this.tipoPrecioVentaEnum.PRECIO_UNITARIO;
          detalle.afectoIGV = false;
  ...
  onSubmit(form: ComprobantePago) {
    ...
    if (this.dataSource.data.length == 0) {
      this.messageService.msgWarning("Ingrese el detalle de la Factura", () => { });
      return;
    }
  ...
  openDialogNewCliente() {
    ...
    const dialogRef = this.dialog.open(NewFacturaClienteComponent, {
      width: "800px",
      disableClose: true,
      data: cliente,
    });
