...
@Component({
  selector: 'app-new-retencion-cliente',
  templateUrl: './new-retencion-cliente.component.html',
  styleUrls: ['./new-retencion-cliente.component.scss']
})
export class NewRetencionClienteComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<NewRetencionClienteComponent>,
  ...
  ngOnInit() {
    ...
    this.tipoDocumentoIdentidadSercice.getTipoDocumentoIdentidades().subscribe(
      (response) => {
        if (response.success) {
          this.tipoDocumentos = response.data.filter(x => x.tipoDocumentoIdentidadId == this.tipoDocIdentidadEnum.RUC);
        }
      });
  }
