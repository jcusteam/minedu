...
@Component({
  selector: "app-new-unidad-ejecutora",
  templateUrl: "./new-unidad-ejecutora.component.html",
  styleUrls: ["./new-unidad-ejecutora.component.scss"],
})
export class NewUnidadEjecutoraComponent implements OnInit {
  // Enun
  tipoDocIdentidadEnum = TipoDocIdentidadEnum;
  form: FormGroup;

  constructor(
    public dialogRef: MatDialogRef<NewUnidadEjecutoraComponent>,
  ...
  onSubmit(form: UnidadEjecutora) {
    ...
    this.messageService.msgConfirm(MESSAGES.FORM.CONFIRM_REGISTER,
      () => {
        ...
        this.UnidadEjecutoraService.createUnidadEjecutora(unidadEjecutora).subscribe(
          (response) => {
            var message = response.messages.join(",");
