...
import { UnidadMedida, UnidadMedidaFilter } from "../models/unidadmedida";
...
export class UnidadMedidaService {
  API_URL =environment.url.apiGateway + environment.url.apiEndPoints.suiteKong + environment.url.apiEndPoints.unidadMedida;

  constructor(
    private http: HttpClient  ) {}

  getUnidadMedidasFilter(
    filter: UnidadMedidaFilter
  ): Observable<IStatusResponse<IPagination<UnidadMedida>>> {
    ...
    return this.http.get<any>(`${this.API_URL}/paginar`, { params,headers }).pipe(
  ...
  getUnidadMedidas() {
    ...
    return this.http
      .get<IStatusResponse<UnidadMedida[]>>(`${this.API_URL}`,{headers})
      .pipe(
        map((response: IStatusResponse<any>) => {
          if (response.success) {
            if (response.data != null) {
  ...
  getUnidadMedidaById(id: number) {
    ...
    return this.http
      .get<IStatusResponse<UnidadMedida>>(`${this.API_URL}/` + id,{headers})
      .pipe(
        map((response: IStatusResponse<any>) => {
          if (response.success) {
            if (response.data != null) {
  ...
  createUnidadMedida(unidadMedida: UnidadMedida) {
    let datehex = Tools.decimalToHexDate();
    let authToken = {
      token: CryptoAes.encryptStringAES256(datehex),
    };
    ...
    let form = {
      data: CryptoAes.encryptAES256(unidadMedida),
    };
    return this.http
      .post<IStatusResponse<UnidadMedida>>(this.API_URL, form,{headers})
      .pipe(
        map((response: IStatusResponse<any>) => {
          if (response.success) {
            if (response.data != null) {
  ...
  updateUnidadMedida(unidadMedida: UnidadMedida) {
    ...
    return this.http
      .put<IStatusResponse<any>>(
        `${this.API_URL}/` + unidadMedida.unidadMedidaId,
        form,{headers}
      )
      .pipe(
        map((response: IStatusResponse<any>) => {
