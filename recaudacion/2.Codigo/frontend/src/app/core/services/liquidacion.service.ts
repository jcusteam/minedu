import { LiquidacionEstado } from './../models/liquidacion';
import { Injectable } from "@angular/core";
import { HttpClient, HttpHeaders, HttpParams } from "@angular/common/http";
import { environment } from "src/environments/environment";
import { Liquidacion, LiquidacionFilter } from "../models/liquidacion";
...
export class LiquidacionService {
  API_URL =environment.url.apiGateway + environment.url.apiEndPoints.suiteKong + environment.url.apiEndPoints.liquidacion;

  constructor(private http: HttpClient) { }

  getLiquidacionesFilter(
    filter: LiquidacionFilter
  ): Observable<IStatusResponse<IPagination<Liquidacion>>> {
  ...
  getLiquidacionById(id: number): Observable<IStatusResponse<Liquidacion>> {
    let datehex = Tools.decimalToHexDate();
    let authToken = {
      token: CryptoAes.encryptStringAES256(datehex),
    };
  ...
  createLiquidacion(liquidacion: Liquidacion) {
    let datehex = Tools.decimalToHexDate();
    let authToken = {
      token: CryptoAes.encryptStringAES256(datehex),
    };
    ...
    let form = {
      data: CryptoAes.encryptAES256(liquidacion),
    };
    return this.http
      .post<IStatusResponse<Liquidacion>>(this.API_URL, form, { headers })
      .pipe(
        map((response: IStatusResponse<any>) => {
          if (response.success) {
            if (response.data != null) {
  ...
  updateLiquidacion(liquidacion: Liquidacion) {
    let datehex = Tools.decimalToHexDate();
    let authToken = {
      token: CryptoAes.encryptStringAES256(datehex),
    };
    ...
    return this.http
      .put<IStatusResponse<any>>(
        `${this.API_URL}/` + liquidacion.liquidacionId,
        form,
        { headers }
      )
      .pipe(
  ...
  updateEstadoLiquidacion(liquidacionEstado: LiquidacionEstado) {
    let datehex = Tools.decimalToHexDate();
    let authToken = {
      token: CryptoAes.encryptStringAES256(datehex),
    };
    ...
    let form = {
      data: CryptoAes.encryptAES256(liquidacionEstado),
    };
    return this.http
      .put<IStatusResponse<any>>(
        `${this.API_URL}/estado/` + liquidacionEstado.liquidacionId,
        form,
        { headers }
      )
      .pipe(
